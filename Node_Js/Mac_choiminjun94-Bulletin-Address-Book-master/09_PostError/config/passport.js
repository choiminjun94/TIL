...
// .Strategy없이 사용해도 되는 것도 있는데, 다들 붙여주니까 같이 붙여줍시다
...
// passport.serializeUser함수는 login시에 DB에서 발견한 user를 어떻게 session에 저장할지를 정하는 부분입니다. 
// user정보 전체를 session에 저장할 수도 있지만, session에 저장되는 정보가 너무 많아지면 사이트의 성능이 떨어질 수 있고, 
// 회원정보수정을 통해 user object가 변경되더라도 이미 전체 user정보가 session에 저장되어 있으므로 
// 해당 부분을 변경해 주어야하는 등의 문제들이 있으므로 user의 id만 session에 저장합니다.
...
// passport.deserializeUser함수는 request시에 session에서 어떻게 user object를 만들지를 정하는 부분입니다. 
...
passport.use('local-login',
  new LocalStrategy({
    ...
    // 만약 로그인 form의 username과 password항목의 이름이 다르다면 여기에서 값을 변경해 주면 됩니다. 
    // 사실 이 코드에서는 해당 항목 이름이 form과 일치하기 때문에 굳이 쓰지 않아도 됩니다. 
    // 예를들어 로그인 form의 항목이름이 email, pass라면 usernameField : "email", passwordField : "pass"로 해야 합니다
    ...
    function(req, username, password, done) { // 3-2
      User.findOne({username:username})
        ...
        // user model에 설정했던 user.authenticate 함수를 사용해서 입력받은 password와 저장된 password hash를 비교해서 값이 
        // 일치하면 해당 user를 done에 담아서 return하고 (return done(null, user);), 
        // 그렇지 않은 경우 username flash와 에러 flash를 생성한 후 done에 false를 담아 return합니다.
        // (return done(null, false);) user가 전달되지 않으면 local-strategy는 실패(failure)로 간주됩니다.
        {
          ...
          //  게시판 - 계정 비밀번호 암호화(bcrypt) 강의에서 bcrypt로 만든 함수입니다.
          }
